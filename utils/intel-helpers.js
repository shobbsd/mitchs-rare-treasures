...
module.exports = (ownerCount, shopCount, treasureCount) => {
  ...
  const treasures = generateMultiples(treasureCount, generateTreasure, shopCount);
  mkdir('./seed/intel')
    .catch(() => console.log('Overwriting existing files in intel directory'))
    .then(() => {
      return writeFile('./seed/intel/owners.js', generateFileText(owners), 'utf8')
    })
    .then(() => {
      return writeFile('./seed/intel/shops.js', generateFileText(shops), 'utf8')
    })
    .then(() => {
      return writeFile('./seed/intel/treasures.js', generateFileText(treasures), 'utf8')
    })
};
